...
class AuthRepository @Inject constructor(
    ...
    /**
     * Clears local session, stops background tasks, and performs server-side logout.
     */
    suspend fun logout() = withContext(dispatcherProvider.io()) {
        ProtonLogger.d(TAG, "Logging out user...")

        // 1. Disconnect VPN first to prevent background traffic loops
        try {
        ...
        // 2. Notify server to invalidate the session (best effort)
        try {
        ...
        // 3. Clear local state and caches
     ...
     * Handles Captcha verification and prevents duplicate token usage (error 12087).
     */
    suspend fun login(username: String, passwordRaw: String, captchaToken: String? = null): Result<LoginResponse> = authMutex.withLock {
        withContext(dispatcherProvider.io() + authJob) {
            try {
                ...
                // If token was used in Phase 0, do NOT pass it to Phase 1 (prevents 12087 error)
