...
class AmneziaConfigGeneratorImpl @Inject constructor() : AmneziaConfigGenerator {
    override fun buildConfig(
        ...
        // Clamp obfuscation parameters to safe ranges before passing to the native library.
        // Excessively large values can cause buffer overflows in the native Amnezia implementation.
        val safeJc   = obfuscationParams.jc.coerceIn(0, 128)
        val safeJmin = obfuscationParams.jmin.coerceIn(0, 1279)
        val safeJmax = obfuscationParams.jmax.coerceIn(0, 1280)
        val safeS1   = obfuscationParams.s1.coerceIn(0, 1132)
        val safeS2   = obfuscationParams.s2.coerceIn(0, 1188)
        val safeS3   = obfuscationParams.s3.coerceIn(0, 1280)
        val safeS4   = obfuscationParams.s4.coerceIn(0, 1280)
        ...
        val ifaceBuilder = Interface.Builder()
            ...
            .setMtu(1280)
            .setJunkPacketCount(safeJc)
            .setJunkPacketMinSize(safeJmin)
            .setJunkPacketMaxSize(safeJmax)
            .setInitPacketJunkSize(safeS1)
            .setResponsePacketJunkSize(safeS2)
            .setCookieReplyPacketJunkSize(safeS3)
            .setTransportPacketJunkSize(safeS4)
            .apply {
